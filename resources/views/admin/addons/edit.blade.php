@extends('layouts.vertical', ['title' => 'Edit Addon'])
                         ...
                         <form action="{{ route('admin.addon.update', $addon) }}" method="POST">
                              ...
                              <div class="row">
                                   ...
                                   <div class="col-md-6">
                                        <div class="mb-3">
                                             ...
                                             <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" value="{{ old('slug', $addon->slug) }}" required>
                                             @error('slug')
                                                  <div class="invalid-feedback">{{ $message }}</div>
                                             @enderror
                                   ...
                                   <div class="col-md-6">
                                        <div class="mb-3">
                                             ...
                                             <div class="input-group">
                                                  ...
                                                  <input type="number" step="0.01" min="0" class="form-control @error('price') is-invalid @enderror" id="price" name="price" value="{{ old('price', $addon->price) }}">
                                                  @error('price')
                                                       <div class="invalid-feedback">{{ $message }}</div>
                                                  @enderror
                                             </div>
                              ...
                              <div class="mb-3">
                                   ...
                                   <div class="form-check mb-3">
                                        <input class="form-check-input" type="checkbox" id="all_foods" name="all_foods" value="1" {{ old('all_foods', $addon->all_foods) ? 'checked' : '' }}>
                                   ...
                                   <div id="food-selection-container" style="{{ old('all_foods', $addon->all_foods) ? 'display:none' : '' }}">
                                        ...
                                        <div class="mb-3">
                                             ...
                                             <div id="selected-foods" class="d-flex flex-wrap gap-2">
                                                  @if($addon->menus->count() > 0 && !$addon->all_foods)
                                                       @foreach($addon->menus as $menu)
                                                            <span class="badge bg-primary-subtle border border-primary text-primary px-3 py-2 fs-14 d-flex align-items-center gap-2" data-food-id="{{ $menu->id }}">
                                                                 {{ $menu->name }}
                                                                 <input type="hidden" name="food_ids[]" value="{{ $menu->id }}">
                                                                 <button type="button" class="btn-close btn-close-sm" onclick="removeFood({{ $menu->id }})" style="font-size: 0.75rem;"></button>
                                                            </span>
                                                       @endforeach
                                                  @else
                              ...
                              <div class="d-flex gap-2">
                                   <button type="submit" class="btn btn-primary">
                                        <i class="ri-save-line me-1"></i> Update Addon
                                   </button>
...
document.addEventListener('DOMContentLoaded', function() {
     ...
     @if(!$addon->all_foods && $addon->menus->count() > 0)
          selectedFoods = [
               @foreach($addon->menus as $menu)
     ...
     window.removeFood = function(id) {
          ...
          updateSelectedFoodsDisplay();
          // Refresh search results to re-enable the removed food
     ...
     function updateSelectedFoodsDisplay() {
          if (selectedFoods.length === 0) {
               ...
               selectedFoodsContainer.innerHTML = selectedFoods.map(food => `
                    <span class="badge bg-primary-subtle border border-primary text-primary px-3 py-2 fs-14 d-flex align-items-center gap-2" data-food-id="${food.id}">
                         ${food.name}
